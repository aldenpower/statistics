...
tempos <- c(csv$tempo1,csv$tempo2)
...
shapiro.test(residuals(doe.model1))
lillie.test(residuals(doe.model1))

# modelo reduzido
doe.model5 <- lm(tempo_medio ~ altura + comp,
                 data = doe)
summary(doe.model.f)
coef(doe.model.f)

residuals(doe.model.f)
residuals(doe.model1)


# Comparação modelos

#modelos <- c(doe.model1, doe.model2, doe.model3, doe.model4, doe.model5)
#for (i in 1:5){
#  soma <- sum(residuals(modelos[i])*residuals(modelos[i]))
#  melhor <- 1000
#  if (soma < melhor){
#    melhor <- soma
#  }
#}
sum(residuals(doe.model1)*residuals(doe.model1))
sum(residuals(doe.model2)*residuals(doe.model2))
sum(residuals(doe.model3)*residuals(doe.model3))
sum(residuals(doe.model4)*residuals(doe.model4))
sum(residuals(doe.model5)*residuals(doe.model5))
